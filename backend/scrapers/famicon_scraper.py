...
HUBpage = 'https://en.wikipedia.org/wiki/List_of_Family_Computer_games'
...
def getAllRows(url):
	...
	table = page_soup.find('table', {'class': 'wikitable plainrowheaders sortable'})
...
def validateRows(rows):
	...
	for row in rows:
		try:
			...
			try:
				if (int(cells[2].get_text().split(',')[1]) < 2003):
...
def extractData(rows):
	...
	for row in rows:
		...
		# release year
		try:
			release_year = (cells[2].get_text().split(',')[1]).replace('\n', '')
			release_yearRead += 1
		except IndexError:
...
def main():
	...
	OutputFile = open('../csvs/famicon.csv','a', encoding = "utf_16")
	...
	for entry in data:
		outputString = outputformat.format(id = entry[0],name = entry[1],release_year = entry[2],developers = entry[3],publishers = entry[4],img_url = entry[5],genres = entry[7], description = entry[8], src_url=entry[6], console = "Famicon")
