...
#This script specifically scrapes all the games at THIS wikipedia article: List of ZX Spectrum games
...
HUBpage = 'https://en.wikipedia.org/wiki/List_of_ZX_Spectrum_games'
...
def extractData(rows):
	...
	for row in rows:
		...
		# release year
		release_year = (cells[4].get_text()).replace('\n', '')
		release_yearRead += 1
		...
		# publisher
		publisher = cells[1].get_text().replace('\n', '')
		publisherRead += 1
		if (len(publisher) <= 2):
			publisher = 'n/a'
			publisherRead -= 1
...
def main():
	...
	OutputFile = open('../csvs/zx_spectrum.csv','a', encoding = "utf_16")
	...
	for entry in data:
		outputString = outputformat.format(id = entry[0],name = entry[1],release_year = entry[2],developers = entry[3],publishers = entry[4],img_url = entry[5],genres = entry[7], description = entry[8], src_url=entry[6], console = "ZX Spectrum home computer")
