...
#	MainText = "<table class='wikitable'>Hello my name is <a>Pedro</a>, and I like crayons</table>"
	...
	for row in tableData:
		...
		#	if Pubs == "":
		#		Pubs = 'n/a'
			year = cleanup_year(data[4].text_content())
			Genres = data[5].text_content().replace("\n", "").replace(" ", "").split(',')
			SearsName = data[1].text_content().replace("\n", "")
			i+=1
			...
			if Genres == None:#genres from the n64 list page seem to be more descriptive, so they will be default
				Genres = Genres2
			if SearsName != '-':
				Discription=Discription+'<br /> Was sold by sears under the name '+SearsName
			outputString = outputformat.format(id = i,name = Name,release_year = year,developers = Devs,publishers = Pubs,img_url = Picture,genres = Genres, description = Discription, src_url=URL, console = "A2600")
