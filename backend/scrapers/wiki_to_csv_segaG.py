...
def investigate_further(Target):
	...
	Genres = [item for item in Genres if (item.replace(',','').replace(' ','') != '')]
	if Genres == "[]":
		Genres = "n/a"
...
#	MainText = "<table class='wikitable'>Hello my name is <a>Pedro</a>, and I like crayons</table>"
	...
	for row in tableData:
		...
		if (len(data)>5):
			...
			print(Name)
			Devs = data[0].text_content().replace("\n", "")
			if Devs == "":
				Devs = 'n/a'
			Pubs = data[1].text_content().replace("\n", "")
			if Pubs == "":
				Pubs = 'n/a'
			JPYear = cleanup_year(data[2].text_content())
			NAYear = cleanup_year(data[3].text_content())
			PALYear = cleanup_year(data[4].text_content())
			OtherYear = cleanup_year(data[5].text_content())
			...
			if (PALYear < year and PALYear!='n/a'):
				year = PALYear
			if (OtherYear < year and OtherYear!='n/a'):
				year = OtherYear
			if (year == '9999'):
				year = 'n/a'
			URL, Picture, Genres, Discription = investigate_further(Link)	#get image, genres, and descriptions
			i+=1
			outputString = outputformat.format(id = i,name = Name,release_year = year,developers = Devs,publishers = Pubs,img_url = Picture,genres = Genres, description = Discription, src_url=URL, console = "seggen")
