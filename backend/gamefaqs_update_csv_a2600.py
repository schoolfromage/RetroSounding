...
def investigate_further(Target, needPic, needDesc):
	...
	if needDesc:	
		Discription = store.xpath("//h2[contains(text(),'Description')]/../../div")[1].text_content().strip()
	else:
		Discription="n/a"
	if needPic:
		time.sleep(0.5)
...
#	MainText = "<table class='wikitable'>Hello my name is <a>Pedro</a>, and I like crayons</table>"
	...
	for row in tableData:
		...
		if name in gameIDs:#ie: if already recorded
			...
			Pubs = 'n/a'
			year = 'n/a'
			URL, Picture, Genres, Discription = investigate_further(Link, needDescriptions[name], needPics[name])	#get image, genres, and descriptions
...
if (manageArgs()==0):
	...
	Scrape(OutputFile)
	OutputFile.close()
	#URL, Picture, Genres, Description = investigate_further("/atari2600/584539-acid-drop", True, True)
	#print(URL)
	#print(Picture)
	#print(Genres)
	#print(Description)